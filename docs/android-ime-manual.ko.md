...
- `scripts/interaction-matrix.sh`(에뮬레이터, 시험용 빌드, 약 9분): 진짜 편집기와 진짜 IME 창만 답할
  수 있는 칸 — 두 모드의 진짜 Termux, 띠의 뷰 트리, 노트, 물리 수정자를 누른 채 액션바 터치, 클립
  목록, 키패드. PASS/FAIL 표를 찍고 실패한 칸의 수로 끝난다.
...
- **액션바가 키 높이를 깎고 있었다.** 붙인 상태에서 바와 키가 한 높이를 나눠 써서, 바를 켜면 모든
  ...
  로 늘었다. 이제 바는 키 **위에 더해진다**. 크기를 사용자가 정한 떠 있는 판만 자기 높이를 나눠
  쓴다.
