...
### 15.66 아무도 누르고 있지 않은 수식키

**벌어진 일.** Shift 는 손가락이 닿는 순간 걸린다(§15.37). 그런데 떼지 못한 누름 — 손가락이 자판 밖으로
미끄러지거나, 창이 사라지거나, 밑에서 페이지가 다시 그려지거나 — 은 타이머만 버렸다. 걸어 둔 래치는 그대로
남아 다음의 상관없는 키가 대문자가 됐다. Ctrl·Alt·Meta 도 같았고, 잠금을 **푼** 탭이 취소되면 잠금은 풀린
채로 남았다(검토 R11). 물리 쪽도 마찬가지였다. 키보드를 뽑을 때나 필드가 바뀔 때 눌려 있던 수식키를 놓지
않아, 키보드는 Ctrl 이 눌려 있다고 계속 믿었고 액션바의 화살표가 Ctrl+화살표로 나갔다(R17).

**고친 방법.** 누름은 그 전에 래치가 무엇이었는지 기억하고, 취소된 누름은 그것을 되돌린다. 꺼져 있었으면
꺼지고, 잠겨 있었으면 잠긴다. 손가락이 눌려 있는 동안 키를 쳤다면 화음은 일어난 것이므로, 뗐을 때처럼
원샷을 쓴다. 다른 손가락이 같은 수식키를 아직 쥐고 있으면 그 손가락이 상태의 주인이다. 페이지를 다시 그릴
때는 그 위의 터치를 취소하고, 이제 그것은 되돌린다는 뜻이다. 물리 수식키는 키 코드가 아니라 **키보드별로**
쥐므로 한 키보드의 키 뗌이 다른 키보드의 것을 놓지 못한다. 사라진 키보드는 자기 키를 데려가고, 필드 경계는
전부 비우며, 평범한 키가 무엇을 싣고 있는지 말해 주는 것을 믿어 오지 않은 키 뗌을 스스로 낫게 한다.

**규칙.** 누른 쪽이 주인이다. 뗌으로 끝나지 않는 누름은 누름을 되돌리는 것으로 끝나고, 수식키는 그것을 쥐고
있는 키보드 — 또는 손가락 — 의 것이다.

### 15.67 아직 부모가 있던 패널

**벌어진 일.** 클립보드 목록과 메모장은 서비스의 필드에 담겨 있어서, 입력 뷰를 다시 만들어도 담긴 내용이
남는다. 그런데 다시 만들 때마다 같은 패널을 새 틀에 넣었고, 화면에서는 내려갔지만 사라지지는 않은 옛 틀이
여전히 그 부모였다. Android 는 이것을 `IllegalStateException: The specified child already has a parent` 로
거부하고 IME 가 멈췄다. 패널이 열린 채 다시 만들면 무엇이든 그랬다. 클립보드가 열린 채 메모를 누르거나 화면을
돌리면 그랬다(이슈 #12). 뒤에는 결함이 하나 더 있었다. 두 필드가 모두 차 있으면 빌드는 클립보드를 보여 주고,
메모장은 그 뒤에 보이지 않게 앉아 있다가 다음 메모 누름이 아무도 볼 수 없는 패널을 닫았다.

**고친 방법.** 담아 둔 패널은 새 틀에 들어가기 전에 옛 부모에서 떨어진다(`detached`). 그리고 두 패널은 서로
자리를 바꾼다. 하나를 열면 다른 하나를 치운다. 메모장은 자기를 닫을 때처럼 먼저 저장한다.

**규칙.** 틀보다 오래 사는 뷰는 다시 놓기 전에 떼어 낸다. 그리고 화면의 한 자리에는 하나만 있다. 패널을 열면
그 자리에 있던 것을 닫는다. 플래그 두 개를 켜 둔 채 빌드가 둘 중 하나를 고르게 두지 않는다.

### 15.68 화면에 없는 뷰에 입력되던 검색

**벌어진 일.** 음성 기호 검색을 닫으면 후보 목록 없이 입력 뷰를 다시 만들었지만, `hanjaView` 는 옛 목록을
계속 가리켰다. 다음 검색은 목록이 "이미 떠 있다"고 보고 그것을 갱신한 뒤 다시 만들기를 건너뛰었다. 그래서
아무것도 나타나지 않는데 검색은 열려 있어 모든 글자를 검색어로 받았다. 사용자가 IME 를 떠날 때까지 키보드는
Shift 가 걸린 채 멈춘 것처럼 보였고 아무것도 입력되지 않았다(이슈 #13).

**고친 방법.** 빌드는 매번 지난 빌드의 후보 뷰를 잊는 것으로 시작한다. 후보를 보여 주는 분기는 자기 것을
새로 만든다. 이제 이 필드는 언제나 "화면에 있는 트리 안의 목록"을 뜻한다.

**규칙.** 뷰를 가리키는 필드는 지금 트리 안의 뷰를 가리킨다. 다시 만들 때 남겨질 수 있다면 다시 만들 때 비운다.
"이미 보이고 있다"는 물음에 보이지 않는 뷰가 답하게 두지 않는다.

### 15.69 멈춘 키보드, 움직이는 패널

**벌어진 일.** 움직임 없음(이슈 #15)은 키보드 뷰와 액션 바에 넣었다. 누름 음영도, 번쩍임도, 글자 상자도 없다.
요청자에게 보낸 답글은 잠깐 보였다가 사라지는 것이 없다고 했다. 메모장은 그렇지 않았다. 버튼은 플랫폼의
테두리 없는 버튼이라 누를 때마다 물결을 그렸고, 체크 상자는 체크 표시를 그려 넣었고, 메모장과 클립보드의
목록은 끝까지 스크롤하면 빛을 냈다. 움직임 없음을 켠 빈 메모장에서 시험이 이런 것을 열아홉 개 셌다.

**고친 방법.** 메모장과 클립보드도 같은 스위치를 읽는다. 스위치가 켜지면 테두리 없는 버튼과 체크 상자는
물결 배경을 잃고(여백은 유지), 체크 표시는 바뀌는 즉시 끝 상태로 건너뛰며, 스크롤 뷰는 끝 빛을 그리지 않는다.

**규칙.** 표시 스위치는 그것을 만든 한 곳이 아니라 IME 가 그리는 모든 면에 관한 것이다. 플랫폼 위젯은 자기
움직임을 데리고 온다. 스위치를 켠 채 패널마다 물결 배경, 움직이는 드로어블, 오버스크롤을 세어서 찾는다.
...
- [ ] 원격 데스크톱 클라이언트가 배치 끝이 아니라 조작마다 커서를 보고해도, 한 음절은 한 음절로 남는다
  (§15.65).
- [ ] Shift 에서 손가락이 미끄러지거나 그 밑에서 페이지가 다시 그려져도 Shift 는 있던 대로 남고, 화음 중
  뽑힌 키보드가 액션바 화살표를 더는 수식하지 않는다(§15.66).
