...
페이지에서 동일하게 고정된다: `Ctrl · Meta · Alt · Tab · space · !# · 🌐`. 메뉴와 키패드 자판은 제
키를 갖지 않는다 — 🌐 와 `!#` 를 길게 누르면 열리며, 각 키 구석의 작은 `m` 과 `p` 가 그 사실을
알린다. 그 둘이 쓰던 자리는 다른 것으로 채우지 않고 비워 두었다. 12키 자판 둘은 같은 뼈대를 다른
모양으로 쓴다 — 수정자가 맨 왼쪽 열을 차지하고 둘째 열은 비어 있으며, 한글 키는 모두 2칸이고,
오른쪽 열이 백스페이스·스페이스·마침표와 엔터를 맡고 `!#` 와 🌐 가 맨 아랫줄을 닫는다.
