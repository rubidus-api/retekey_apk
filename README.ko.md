...
- **입체적인 둥근 키**와 눌림 피드백 — 누른 키에 색이 들어오고 자판 전체가 한 번 깜빡이며, 진동과
  소리까지 각각 따로 조절.
...
글자 키를 길게 누르면 반복 입력 대신 문자가 입력되며, 세 줄이 각자 하나의 자판을 이룬다 — 첫째 줄은
숫자 `1`–`0`, 둘째 줄은 `!@#$%^&*`, 셋째 줄은 `_ - : = ' " ?`. 한글과 영문이 이를 공유하므로 손이
익힌 자리에 그대로 있다. 스페이스·엔터·백스페이스·화살표는 대체 문자가 없어 누르고 있으면 반복된다.
...
줄어들지도 않는다. 모드와 패널 위치는 저장되며, 화면을 돌리면 패널이 사라지는 대신 비율로 옮겨진다.
...
후보는 훈음(家 → 집 가)과 함께 별도의 창에 격자로 표시된다. 키보드의 일부가 아니므로 화면 키보드,
화면에 키보드가 아예 없는 외장 키보드, 플로팅 패널 어디서나 똑같이 뜬다. 후보를 탭하거나 숫자키 **1**–**9**로
