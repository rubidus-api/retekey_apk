...
| | |
|---|---|
| ![안드로이드 10 의 2벌식](assets/keyboard-korean.png) | ![안드로이드 10 의 한자 후보](assets/keyboard-hanja.png) |
| 2벌식 — 입체 키와 모서리의 홀드 문자 | 한자 창 — 키보드 폭, 훈음이 글자 오른쪽에, 한 페이지 아홉 개 |
| ![안드로이드 10 의 메뉴 자판](assets/keyboard-menu.png) | ![안드로이드 10 의 플로팅 키보드](assets/keyboard-floating.png) |
...
**두벌식** — `!#` 옆의 漢 이 방금 친 글자를 한자로 바꾼다:

![두벌식 자판](assets/keyboard-korean.png)

**QWERTY** — 스페이스 옆에 `Esc`. ssh 로 vi 를 쓸 때를 위한 것:

![QWERTY 자판](assets/keyboard-qwerty.png)

**드보락** — 하단 행은 같고, 글자 블록만 제 7/10/9 배치:

![드보락 자판](assets/keyboard-dvorak.png)

**천지인** — Alt 옆에 다음, Tab 옆에 漢, ㅇㅁ 양옆에 `.,` 와 `!?`:

![천지인 자판](assets/keyboard-cheonjiin.png)

**나랏글** — 맨 아랫줄에 획과 쌍, Tab 옆에 漢:
...
**`!#` 를 길게 누르면** 특수키 자판이 열린다. 오른쪽 키패드와 특수키로 구성되며, `Num` 이 **켜진**
채로 — 숫자가 나온 채로 — 열린다. 넘버락이 켜져 있으면 숫자, 다른 키보드와 같다. 숫자와 `+ - = .` 는
텍스트를 입력하고 `Esc`·`PrtSc`·`ScrLk`·`Pause`·`Menu` 는 키 이벤트를 보낸다:

![특수키 자판, 숫자](assets/keyboard-keys.png)

`Num` 을 **끄면** 키패드가 화살표·이동키로 바뀐다 — Home/End, PgUp/PgDn, Ins, 앞으로 지우기:

![특수키 자판, 화살표](assets/keyboard-keys-arrows.png)

`Fn` 은 페이지 전체를 기능키·미디어키로 바꾼다. F1–F12 는 키 이벤트를 보내고, 안드로이드 키코드가 없는
F13–F15 와 미디어키·뒤로가기는 비활성 상태로 둔다:

![특수키 자판, Fn 페이지](assets/keyboard-keys-fn.png)

### 메뉴 자판

**🌐 를 길게 누르면** 열린다. 오른쪽 절반이 편집하는 손이다 — 안쪽 끝에 복사·잘라내기·붙여넣기,
가운데에 전체선택을 두고 화살표가 십자로, 그 둘레에 이동키, 그리고 엄지가 보지 않고 닿는 구석에 설정.
왼쪽 절반은 키보드 자신에 관한 것들이다.

![메뉴 자판](assets/keyboard-menu.png)

| | 키 | 하는 일 |
|---|---|---|
| ↶ | 실행취소 | 마지막 편집을 되돌린다 |
| ↷ | 다시실행 | 되돌린 것을 다시 한다 |
| 📅 | 날짜 | 현재 날짜와 시각을 글자로 넣는다 |
| ☺ | 이모지 | *아직 없음 — 흐리게 그려 둔 자리* |
| 🗒 | 클립보드 | *아직 없음* |
| ⧉ | 복사 | 선택 영역을 복사 |
| ✂ | 잘라내기 | 선택 영역을 잘라내기 |
| 📋 | 붙여넣기 | 클립보드를 붙여넣기 |
| ⬚A | 전체선택 | 입력란 전체를 선택 |
| size− / size+ | 높이 | 한 번에 한 단계씩 줄이고 키운다. 방향별로 기억된다 |
| ⌨↔ | 자판 전환 | 시스템 입력기 선택창을 연다 |
| ⌨⚙ | 자판 관리 | 시스템의 키보드 켜기/끄기 화면을 연다 |
| ❐ | 플로팅 | 플로팅 패널을 켜고 끈다 |
| ◀\| / \|↔\| | 한손 모드 | *아직 없음* |
| ◐ | 테마 | *아직 없음* |
| ★1 / ★2 | 사용자 지정 | *아직 없음* |
| Set | 설정 | ReteKey 자체 설정 화면을 연다 |
| ← ↑ → ↓ | 화살표 | 커서를 옮긴다. 길게 누르면 반복된다 |
| Home / End | 줄 끝 | 줄의 처음·끝으로 |
| PgUp / PgDn | 페이지 | 한 페이지씩 |
| Ins | 삽입 | Insert 키 |
| Del | 앞으로 지우기 | 커서 뒤를 지운다 |
| ABC | 돌아가기 | 글자 자판으로 돌아간다 |

흐린 키들은 그려져 있지만 아무 일도 하지 않는다. 기능이 들어갈 자리를 미리 잡아 둔 것이라, 나중에
생겨도 페이지가 밀리지 않는다.

## 플로팅 키보드

메뉴 자판의 **❐ 플로팅** 키를 누르면 키보드가 화면 아래에 붙는 대신 앱 위에 뜨는 반투명 패널이 된다.
...
고를 수 있고, 헤더의 `‹`/`›` 또는 키보드의 `←`/`→`·`PageUp`/`PageDown` 으로 페이지를 넘긴다.
**✕** 를 누르면 변환하지 않고 빠져나온다(`Esc` 와 같다). 입력한 글자는 친 그대로 남는다 — 취소가
아니라 나가기다.

![한자 후보 창](assets/keyboard-hanja.png)

> **알려진 제약.** 화면 내비게이션 바가 있는 기기에서는 패널의 마지막 줄이 그 위로 걸칠 수 있다.
> 후보는 보이고 숫자키로도 고를 수 있지만 그 줄은 탭이 안 먹을 수 있다. 이 패널은 키보드 창의 자식
> 창이라 예전에는 아예 잘려 나갔는데(줄 하나가 통째로 사라졌다) 그것은 고쳤고, 시스템 바를 피해
> 놓는 것은 아직이다.
